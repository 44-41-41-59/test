...
router
  .route('/store')
  .post(bearer('registered'), permissions('createStore'), addStore)
  .get(getAllStores);
// get all pending stores in the admin dashboard // bearer and permission to get admin dashboard
router
  .route('/store/admin/dashboard')
  .get(bearer('registered'),permissions('readPendingStores'),getPendingStores);
...
router
  .route('/store/:id')
  .get(getOneStore)
  .put(bearer('registered'), permissions('updateStore'), editStore)
  .delete(bearer('registered'), permissions('deleteStore'), deleteStore)
  .patch(bearer('registered'), permissions('updateStoreStatus'), editStore);
